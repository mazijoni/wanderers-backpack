...
public record BackpackState(int cols, int rows, boolean slotsUnlocked)
{
    ...
    {
        // Note: cols/rows are no longer compared against Config.BACKPACK.inventory.size - each
        // backpack tier now has its own fixed size (see BackpackItem), so that global config no
        // longer applies and comparing against it would incorrectly invalidate every container.
        return this.slotsUnlocked != Config.BACKPACK.inventory.slots.unlockAllSlots.get();
    }
}
