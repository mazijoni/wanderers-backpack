...
public class ClientEvents
{
    ...
    private static boolean onInteraction(boolean attack, boolean use, boolean pick, InteractionHand hand)
    {
        ...
        if(tryPickpocket(mc, hand))
            return true;

        // Sneak + right click while holding a backpack (and not pickpocketing anyone) opens it
        // directly, without equipping it
        if(mc.player.isShiftKeyDown())
        {
            ItemStack heldStack = mc.player.getMainHandItem();
            if(heldStack.getItem() instanceof BackpackItem)
            {
                int index = BackpackHelper.INDEX_INVENTORY_START + mc.player.getInventory().selected;
                Network.getPlay().sendToServer(new MessageOpenBackpack(index));
                return true;
            }
        }
    ...
    private static boolean tryPickpocket(Minecraft mc, InteractionHand hand)
    {
