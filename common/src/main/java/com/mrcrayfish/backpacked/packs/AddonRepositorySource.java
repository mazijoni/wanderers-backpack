...
public class AddonRepositorySource implements RepositorySource
{
    ...
    public void loadPacks(Consumer<Pack> consumer)
    {
        try
        {
            Constants.LOG.info("Looking for Wanderer's Backpack addons in '{}'", this.path.getFileName().toString());
            int[] counter = {0};
            ...
            Constants.LOG.info("Found {} Wanderer's Backpack addons", counter[0]);
        }
        catch(IOException e)
        {
            Constants.LOG.warn("Failed to list Wanderer's Backpack addon in {}", this.path, e);
        }
    }
}
