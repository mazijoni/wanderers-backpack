...
{
    public static final String FILE_NAME = "wanderersbackpack_addon.mcmeta";
    ...
    public static Optional<Optional<AddonMetadata>> readAddonMetadata(PackLocationInfo info, Pack.ResourcesSupplier resourcesSupplier, PackType type)
    {
        try
        {
            try(PackResources resources = resourcesSupplier.openPrimary(info))
            {
                ...
                if(value.isEmpty())
                {
                    Constants.LOG.error("Failed to read metadata for Wanderer's Backpack addon '{}'. Skipping resource", info.id());
                ...
                {
                    Constants.LOG.error("Skipping Wanderer's Backpack addon '{}' as it was designed for a newer version of Wanderer's Backpack. Expected addon_format {} or lower, found {}", info.id(), Constants.ADDON_FORMAT, section.addonFormat);
                    return Optional.of(Optional.empty());
                }
    ...
    {
        IoSupplier<InputStream> supplier = resources.getRootResource("wanderersbackpack_addon.mcmeta");
        if(supplier != null)
        {
    ...
    private record AddonMetadataSection(Component name, Component description, Component author, int addonFormat, int assetsFormat, int dataFormat)
    {
        ...
        public static final MetadataSectionType<AddonMetadataSection> TYPE = MetadataSectionType.fromCodec("wanderersbackpack_addon", CODEC);
    }
}
