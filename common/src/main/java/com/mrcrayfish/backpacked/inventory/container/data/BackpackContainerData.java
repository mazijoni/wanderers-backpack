 ...
 * Author: MrCrayfish
 */
public record BackpackContainerData(int backpackIndex, int columns, int rows, boolean owner, UnlockableSlots slots, Pagination pagination, Augments augments, UnlockableSlots bays, int augmentSlots) implements IMenuData<BackpackContainerData>
{
    public static final StreamCodec<RegistryFriendlyByteBuf, BackpackContainerData> STREAM_CODEC = StreamCodec.of((buf, data) -> {
        ...
        ByteBufCodecs.INT.encode(buf, data.augmentSlots);
    }, buf -> {
        ...
        int augmentSlots = buf.readInt();
        return new BackpackContainerData(backpackIndex, columns, rows, owner, slots, pagination, augments, bays, augmentSlots);
    });
