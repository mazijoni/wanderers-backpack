...
public class ManagementInventory implements Container
{
    ...
    /**
     * Backpacks can be equipped in the chest armor slot and, if Curios is installed, the Curios
     * "back" slot - so there are always exactly two equip slots (unlike the original multi-backpack
     * array, which allowed up to 9 configurable). This is only used to size the small "which of your
     * equipped backpacks goes on the shelf" picker; browsing every backpack in the inventory uses
     * {@link com.mrcrayfish.backpacked.BackpackHelper#getTotalBackpackIndices} instead.
     */
    public static int getMaxEquipable()
    {
        return BackpackHelper.INDEX_INVENTORY_START;
    }
}
